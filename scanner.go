...
// The DSLScanner contains a reference to the user scan function, the
...
func newScanner(sf ScanFunc, r *bufio.Reader, l logger) *DSLScanner {
	s := &DSLScanner{
		fn:      sf,
		r:       r,
		l:       l,
...
type ScanFunc func(*DSLScanner) Token
...
func (s *DSLScanner) Expect(expect ExpectRune) {
...
func (s *DSLScanner) Call(fn func(*DSLScanner)) {
	s.log("Call", prefixNewline)
	s.callFn(fn)
}
...
func (s *DSLScanner) Match(matches []Match) {
	if s.tok.ID != "" {
		return
	}
...
type scanner interface {
	scan() (Token, *Error)
	newError(code ErrorCode, err error) *Error
}

// scan is the entry point from the parser.
func (s *DSLScanner) scan() (Token, *Error) {
	s.init()
...
func (s *DSLScanner) newError(code ErrorCode, err error) *Error {
...
func (s *DSLScanner) consume(rn rune, skip bool, found1orMore bool) {
	if !found1orMore {
...
func (s *DSLScanner) log(msg string, indent indent) {
	s.l.log(msg, indent)
}

// getLine is used to scan the rest of the current line to display in the Error
func (s *DSLScanner) getLine() string {
	var numRunes int
